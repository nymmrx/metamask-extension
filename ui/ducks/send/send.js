...
const slice = createSlice({
  ...
  reducers: {
    ...
    useBasicGasEstimation: (state) => {
      // Show the basic gas estimation buttons in the send page
      state.gas.mode = 'BASIC';
    },
    useAdvancedGasEstimation: (state) => {
      // Show the advanced gas price/limit fields in the send page
      state.gas.mode = 'ADVANCED';
    },
    ...
    validateRecipientUserInput: (state, action) => {
      ...
      if (
        ...
        if (
          !isValidAddress(recipient.userInput) &&
  ...
  return async (dispatch, getState) => {
    ...
    if (stage === SEND_STAGES.EDIT) {
      ...
      try {
        ...
        console.log(token);
        console.log(address, value, txParams);
